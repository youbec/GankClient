...
public class Front_endFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private MainBeanView mainBeanView = new MainBeanView() {
        ...
        public void onSuccess(final MainBean mainBean) {
            ...
            String front = jsonArray.toJSONString(results);
            ACache.get(getActivity()).put("front",front,60*60);
...
//            mainAdapter.isFirstOnly(false);
            ...
            mainAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
                ...
                public void onLoadMoreRequested() {
                    isFirstPage = false;
                    page++;
                    mainPresenter.getMainInfo("前端", 10, page);
                }
            }, recyclerView);
        }
        ...
        public void onError(String result) {
            ...
            String front = ACache.get(getActivity()).getAsString("front");
            List<ResultsBean> resultsBeen = JSONObject.parseArray(front, ResultsBean.class);
            if (front==null){
                swipeRefreshLayout.setRefreshing(false);
            }else {
                swipeRefreshLayout.setRefreshing(false);
