...
public class AppFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private MainBeanView mainBeanView = new MainBeanView() {
        ...
        public void onSuccess(final MainBean mainBean) {
            ...
            String app = jsonArray.toJSONString(results);
            ACache.get(getActivity()).put("app",app,60*60);
           ...
           // mainAdapter.openLoadAnimation(BaseQuickAdapter.SCALEIN);
            ...
            mainAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
                ...
                public void onLoadMoreRequested() {
                    isFirstPage = false;
                    page++;
                    mainPresenter.getMainInfo("App", 10, page);
                }
            }, recyclerView);
        }
        ...
        public void onError(String result) {
            ...
            String app = ACache.get(getActivity()).getAsString("app");
            List<ResultsBean> resultsBeen = JSONObject.parseArray(app, ResultsBean.class);
            if (app==null){
                swipeRefreshLayout.setRefreshing(false);
            }else {
                swipeRefreshLayout.setRefreshing(false);
